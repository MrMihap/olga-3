...
int main()
{
	...
	for(FuncType = 1; FuncType <= 3; FuncType++)
	{
		for(i = 0; i < n + 1; i++)
		{
			...
			{
				if(i == j && j == 0)
					B[(n + 1) * i + j] = CalcB(h * (i * h + j * h * cos(AlphaAngle)), j * h * sin (AlphaAngle), 1);

				if(j > 0 && j < n + 1 - 1 && i == 0)
					B[(n + 1) * i + j] = CalcB(h * (i * h + j * h * cos(AlphaAngle)), j * h * sin (AlphaAngle), 2);

				if(j == n + 1 - 1 && i == 0)
					B[(n + 1) * i + j] = CalcB(h * (i * h + j * h * cos(AlphaAngle)), j * h * sin (AlphaAngle), 3);

				if(j == n + 1 - 1 && i > 0 && i < n + 1 - 1)
					B[(n + 1) * i + j] = CalcB(h * (i * h + j * h * cos(AlphaAngle)), j * h * sin (AlphaAngle), 4);

				if(j == n + 1 - 1 &&  i == n + 1 - 1)
					B[(n + 1) * i + j] = CalcB(h * (i * h + j * h * cos(AlphaAngle)), j * h * sin (AlphaAngle), 5);

				if(j > 0 && j < n + 1 - 1 && i == n + 1 - 1)
					B[(n + 1) * i + j] = CalcB(h * (i * h + j * h * cos(AlphaAngle)), j * h * sin (AlphaAngle), 6);

				if(j == 0 && i == n + 1 - 1)
					B[(n + 1) * i + j] = CalcB(h * (i * h + j * h * cos(AlphaAngle)), j * h * sin (AlphaAngle), 7);

				if(j == 0 && i > 0 && i < n + 1 - 1)
					B[(n + 1) * i + j] = CalcB(h * (i * h + j * h * cos(AlphaAngle)), j * h * sin (AlphaAngle), 8);

				if(j > 0 && j < n + 1 - 1 && i > 0 && i < n + 1 - 1)
					B[(n + 1) * i + j] = CalcB(h * (i * h + j * h * cos(AlphaAngle)), j * h * sin (AlphaAngle), 9);
			}
		}
		for(i = 0; i < N; i++)
		{
